...
public interface Direct_Hospital_Request_Order_DetailRepository extends JpaRepository<Direct_Hospital_Request_Order_Detail,Long> {
    ...
    public Integer cansullpyorder(@Param("o_id") Long o_id,@Param("m_id") String m_id, @Param("x") Long x,@Param("med_id") String med_id);

    @Query(value="{call sypply_orders_direct_hospital(:o_d_id, :x, :med, :m_s_id)}",nativeQuery=true)
    public Integer sypplyordersdirecthospital(@Param("o_d_id") Long o_d_id,@Param("x") Long x,@Param("med") String med,@Param("m_s_id") String m_s_id);
