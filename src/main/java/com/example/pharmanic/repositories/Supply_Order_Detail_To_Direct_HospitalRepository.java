...
//    public Integer getDailySupplyToDirectHospital();
    ...
    @Query(value="SELECT date FROM supply_order_detail_to_rdhs union distinct SELECT date FROM supply_order_detail_to_direct_hospital union distinct SELECT date from direct_hospital_request_order union distinct SELECT date from rdhs_hospital_request_order  order by date limit 10;",nativeQuery=true)
    public Date[] getDate();
    ...
    public Integer getDirectYearsSupplyCount();

    @Query(value="SELECT sum(quantity)/1000 FROM direct_hospital_request_order d inner join direct_hospital_request_order_detail dd on d.order_id=dd.order_id where year(date)=year(curdate());",nativeQuery=true)
